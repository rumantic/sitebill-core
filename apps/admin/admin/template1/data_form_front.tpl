...
{if $form_elements.public|count eq 1}
    ...
    {foreach from=$form_elements.public key=tab item=tab_elements}
		{if $bootstrap_version=='3' && $smarty.const.ADMIN_MODE!=1}
					...
					<div class="{if $element.type=='textarea' or $element.type=='textarea_editor' or $element.type=='uploads' or $element.type=='docuploads' or $element.type=='geodata'}col-sm-12{else}col-sm-6{/if}" alt="{$element.name}">
		...
		{foreach name=tbf from=$form_elements.public key=tab item=tab_elements}
			...
			<div role="tabpanel" class="tab-pane fade in{if $smarty.foreach.tbf.iteration==1} active{/if}" id="t{$tab_id}">
				{if $bootstrap_version=='3' && $smarty.const.ADMIN_MODE!=1}
					...
					<div class="row">
					{foreach from=$tab_elements item=element}
					<div class="{if $element.type=='textarea' or $element.type=='textarea_editor' or $element.type=='uploads' or $element.type=='docuploads' or $element.type=='geodata'}col-md-12{else}col-md-6{/if}" alt="{$element.name}">
                    {if $element.type=='captcha'}

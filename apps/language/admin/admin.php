...
class language_admin extends Object_Manager {
    ...
    function __construct($realty_type = false) {
        ...
        if (!$config_admin->check_config_item('apps.language.google_translate_api_key')) {
        ...
        if (!$config_admin->check_config_item('apps.language.autotrans_enable')) {
            $config_admin->addParamToConfig('apps.language.autotrans_enable', '1', 'Активировать автоперевод', 1);
        }
        
        if (!$config_admin->check_config_item('apps.language.autotrans_api')) {
            $config_admin->addParamToConfig('apps.language.autotrans_api', '0', 'API переводов');
        }
        if (!$config_admin->check_config_item('apps.language.yandex_translate_api_key')) {

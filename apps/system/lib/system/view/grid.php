  ...
  <div class="modal-dialog" role="document">
    ...
    <div class="modal-footer"></div>
            ...
            if (count($this->grid_controls) > 0) {
                $rs .= '<th  width="1%"><a class="tags-clear" href="">Очистить</a></th>';
            }
            $rs .= '</tr>';
            $rs .= '</thead>';
            $rs .= '<tbody>';
...
';
                ...
                if(!$disable_mass_delete || $this->batchUpdate || $this->batchActivate){
                    ...
                    if($this->batchUpdate){
                        $rs .= '<button alt="' . $this->grid_object->action . '" class="batch_update btn btn-inverse"><i class="icon-white icon-th"></i> Пакетная обработка</button> ';
                    }
                    if($this->batchActivate){
                        $rs .= '<button alt="' . $this->grid_object->action . '" data-action="activate" class="mass_action btn btn-inverse">Активировать</button> ';
                        $rs .= '<button alt="' . $this->grid_object->action . '" data-action="deactivate" class="mass_action btn btn-inverse">Архивировать</button> ';
                    }
                    $rs .= '</td></tr>';
                }
                /*if (!$disable_mass_delete) {
